...
function findFirstLongString(inputStrings) {
    const result = inputStrings.filter((word) => word.length > 6);
    // @ts-ignore
    return result[0];
}
...
function findAllTheStrings(inputArray) {
    const result = inputArray.filter((word) => typeof(word) === "string");
    return result;
}
...
function findAllPeopleWithALongName(inputObjects) {
    const result = inputObjects.filter((person) => person.firstName.length > 6 || person.secondName.length > 6 );
    return result;
}
...
function areAllNumbersOdd(inputNumbers) {
    const result = inputNumbers.filter((number) => number % 2 !== 0);
        if (result.length === inputNumbers.length){
            return true;
        } else {
            return false;
        }
...
function areAllStringsLong(inputStrings) {
    const result = inputStrings.filter((str) => str.length > 6);
        if (result.length === inputStrings.length){
            return true;
        } else {
            return false;
        }
